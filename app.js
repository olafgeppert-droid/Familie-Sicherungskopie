...
function undo(){ if(!undoStack.length) return; redoStack.push(clone(people)); people = undoStack.pop(); save(); renderTable(); renderTree(); renderTree(); renderTree(); updateUndoRedo(); }
function redo(){ if(!redoStack.length) return; undoStack.push(clone(people)); people = redoStack.pop(); save(); renderTable(); renderTree(); renderTree(); renderTree(); updateUndoRedo(); }
...
function genColor(g){
  const palette = ['#d1f7c4','#fff3b0','#ffc9c9','#e5d0ff','#b2f5ea','#e2e8f0'];
  const n = Math.max(1, parseInt(g||1));
...
function drawNode(svg, x, y, w, h, p){
  ...
  r.setAttribute("fill", genColor(p.Generation));
  g.appendChild(r);
...
// Print
function printTable(){
  const title='Wappenringe der Familie GEPPERT'; window.print(); }
function printTree(){
  const title='Wappenringe der Familie GEPPERT'; window.print(); }
...
window.addEventListener("DOMContentLoaded", ()=>{
  ...
  qs("#btnConfirmDelete").addEventListener("click", (e)=>{
    ...
    // Referenzen auf entfernte Person löschen
    const del = people[idx];
    const base = baseOf(del.Code);
    for(const p of people){
      if(p.ParentCode===base) p.ParentCode="";
      if(p.PartnerCode===base || p.PartnerCode===del.Code) p.PartnerCode="";
      if(p.InheritedFrom===del.Code) p.InheritedFrom="";
    }
    people.splice(idx,1);
...
qs("#btnDoPrint").addEventListener("click", (e)=>{
  e.preventDefault();
  const which = new FormData(qs("#frmPrint")).get("printWhat");
  qs("#dlgPrint").close();
  if(which==="tree") printTree(); else printTable();
});

// Export selection dialog
qs("#btnDoExport").addEventListener("click", async (e)=>{
  e.preventDefault();
  const fd = new FormData(qs("#frmExport"));
  const fmts = fd.getAll("fmt");
  const wantShare = qs("#expShare").checked && (navigator.share || (navigator.canShare && navigator.canShare({ files: [] })));
  qs("#dlgExport").close();
  if(fmts.length===0){ alert("Bitte mindestens ein Format wählen."); return; }
  const files = [];
  if(fmts.includes("json")){
    const blob = new Blob([JSON.stringify(people,null,2)], {type:"application/json"});
    files.push(new File([blob], "wappenringe.json", {type:"application/json"}));
    if(!wantShare) downloadBlob(blob, "wappenringe.json");
  }
  if(fmts.includes("csv")){
    ...
    for(const p of people){ lines.push(cols.map(k => ((p[k]||"")+"").replace(/;/g,",")).join(";")); }
    const blob = new Blob([lines.join("\n")], {type:"text/csv"});
    files.push(new File([blob], "wappenringe.csv", {type:"text/csv"}));
    if(!wantShare) downloadBlob(blob, "wappenringe.csv");
  }
  if(wantShare && files.length){
    try{
      await navigator.share({ title:"Wappenringe Export", text:"Daten sichern", files });
    }catch(err){ console.warn("Share abgebrochen/fehlgeschlagen", err); }
  }
});

// Help modal
qs("#btnHelp").addEventListener("click", async ()=>{
  const m = qs("#dlgHelp"); m.classList.remove("hidden");
  try{
    const resp = await fetch("help.html"); const txt = await resp.text();
    qs("#helpBody").innerHTML = txt;
  }catch(e){ qs("#helpBody").textContent = "Hilfe konnte nicht geladen werden."; }
});
qs("#btnCloseHelp").addEventListener("click", ()=> qs("#dlgHelp").classList.add("hidden"));
qs("#helpBackdrop").addEventListener("click", ()=> qs("#dlgHelp").classList.add("hidden"));
