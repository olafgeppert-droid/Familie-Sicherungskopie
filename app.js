...
function recalcAll(){
  // Auto RingCode + InheritedFrom
  const byCode = Object.fromEntries(people.map(p => [p.Code, p]));
  for(const p of people){
    const base = baseOf(p.Code);
    if(!p.ParentCode){ // root
      p.RingCode = base;
      p.InheritedFrom = "";
    }else{
      const parent = byCode[p.ParentCode];
      p.RingCode = parent ? parent.RingCode : base;
      p.InheritedFrom = p.ParentCode || "";
    }
    if(isPartner(p.Code)){
      const partner = byCode[base];
      p.RingCode = partner ? partner.RingCode : base;
      p.InheritedFrom = base;
    }
