...
// Ensure print buttons are wired
document.addEventListener('DOMContentLoaded', ()=>{
  const btnPrint = document.getElementById('btnPrint');
  if (btnPrint) btnPrint.onclick = openPrintDialog;
  const btnDoPrint = document.getElementById('btnDoPrint');
  if (btnDoPrint) btnDoPrint.onclick = (e)=>{ e.preventDefault(); document.getElementById('dlgPrint').close(); doPrint(); };
});


// ---- Safety bindings for Stats/Reset/Help ----

(function(){const el=document.getElementById('btnStats'); if(el&&!el._bound){
  el._bound=true;
  el.addEventListener('click', (e)=>{ e.preventDefault(); if(typeof openStatistics==='function') openStatistics(); else if(window.showStatisticsDialog) showStatisticsDialog(); else alert('Statistik-Funktion nicht gefunden.'); });
}})();
(function(){const el=document.getElementById('btnReset'); if(el&&!el._bound){
  el._bound=true;
  el.addEventListener('click', (e)=>{ e.preventDefault(); if(confirm('Sollen wirklich alle Personen gelöscht werden?')){ try{ localStorage.removeItem('familyData'); localStorage.removeItem('familyDataVersion'); }catch(_){
  } if(typeof resetAllData==='function') resetAllData(); if(typeof rebuildUI==='function') rebuildUI(); location.reload(); } });
}})();
(function(){const el=document.getElementById('btnHelp'); if(el&&!el._bound){
  el._bound=true;
  el.addEventListener('click', async (e)=>{ e.preventDefault();
    let dlg=document.getElementById('dlgHelp');
    if(!dlg){ dlg=document.createElement('dialog'); dlg.id='dlgHelp'; dlg.style.width='min(900px,90vw)'; dlg.style.maxHeight='85vh'; dlg.style.overflow='auto'; dlg.style.padding='0'; document.body.appendChild(dlg); }
    let header='<div style="display:flex;align-items:center;justify-content:space-between;padding:.6rem 1rem;border-bottom:1px solid #ddd;"><strong>Hilfe & Anleitung</strong><button id="btnCloseHelp" title="Schließen" style="font-size:1rem;border:none;background:none;cursor:pointer;">✖</button></div>';
    let body='';
    try{ const res=await fetch('help.html'); if(res.ok) body=await res.text(); else body='<div style="padding:1rem">Bitte legen Sie eine <code>help.html</code> im Verzeichnis ab.</div>'; }catch(_){
      body='<div style="padding:1rem">Bitte legen Sie eine <code>help.html</code> im Verzeichnis ab.</div>';
    }
    dlg.innerHTML = header + '<div style="padding:1rem">'+body+'</div>';
    dlg.showModal();
