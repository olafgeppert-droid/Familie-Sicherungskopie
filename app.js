...
// --- Robust login glue (upd33) ---
window.addEventListener("DOMContentLoaded", ()=>{
  try{
    const loginBtn = document.getElementById("btnLogin") || document.getElementById("loginBtn");
    const pwd = document.getElementById("pwd");
    if(loginBtn){
      loginBtn.addEventListener("click", (e)=>{ e.preventDefault(); if (typeof doLogin === 'function') doLogin(); });
    }
    if(pwd){
      pwd.addEventListener("keydown", (e)=>{
  ...
  }catch(err){ console.error("Login wiring error:", err); }
});

// --- doLogin fallback (accept demo, clear on fail) ---
(function(){
  const hasDoLogin = (typeof window.doLogin === 'function');
  const orig = window.doLogin;
  window.doLogin = async function(){
    try{
      const pwdEl = document.getElementById("pwd");
      const msgEl = document.getElementById("loginMsg");
      const v = (pwdEl && pwdEl.value ? pwdEl.value.trim() : "");
      if (v === "demo"){
      ...
      if (hasDoLogin) {
        ...
        if (msgEl) { msgEl.textContent = "Falsches Passwort"; msgEl.style.color = "red"; }
        if (pwdEl) { pwdEl.value = ""; pwdEl.focus(); }
      }
    }catch(e){
      const msgEl = document.getElementById("loginMsg");
      if (msgEl) { msgEl.textContent = "Fehler beim Login"; msgEl.style.color = "red"; }
      const pwdEl = document.getElementById("pwd"); if (pwdEl) { pwdEl.value = ""; pwdEl.focus(); }
      console.error(e);
    }
  }
})();
