...
// Draw tree with generation bands and correct lines
function drawTree(data) {
    ...
    data.forEach(person => {
        ...
        box.setAttribute("x", person.x);
        box.setAttribute("y", person.y);
        box.setAttribute("width", 120);
        box.setAttribute("height", 50);
        box.setAttribute("class", "tree-box generation-" + person.generation);
        svg.appendChild(box);

        // Text: Zeile1 Code+Name
        const text1 = document.createElementNS("http://www.w3.org/2000/svg", "text");
        text1.setAttribute("x", person.x + 5);
        text1.setAttribute("y", person.y + 20);
        text1.textContent = person.code + " / " + person.name;
        svg.appendChild(text1);

        // Text: Zeile2 Generation + Geburtsdatum
        const text2 = document.createElementNS("http://www.w3.org/2000/svg", "text");
        text2.setAttribute("x", person.x + 5);
        text2.setAttribute("y", person.y + 40);
        text2.textContent = "Gen " + person.generation + " / " + person.birth;
        svg.appendChild(text2);
    });

    // Linien Eltern-Kinder + Partner (vereinfachte Logik, Platzhalter)
    data.forEach(person => {
        if (person.children) {
            person.children.forEach(childId => {
                ...
                if (child) {
                    ...
                    line.setAttribute("x1", person.x + 60);
                    line.setAttribute("y1", person.y + 50);
                    line.setAttribute("x2", child.x + 60);
        ...
        if (person.partner) {
            const partner = data.find(p => p.id === person.partner);
            if (partner) {
                const line = document.createElementNS("http://www.w3.org/2000/svg", "line");
                line.setAttribute("x1", person.x + 120);
                line.setAttribute("y1", person.y + 25);
                line.setAttribute("x2", partner.x);
                line.setAttribute("y2", partner.y + 25);
                if (person.children && person.children.length > 0) {
                    line.setAttribute("class", "tree-line");
                } else {
                    line.setAttribute("class", "tree-line-dashed");
                }
                svg.appendChild(line);
            }
        }
