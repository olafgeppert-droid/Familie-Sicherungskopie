...
function printTable(){
  ...
  const el = document.querySelector("#peopleTable");
...
async function shareOrDownloadPDF(blob, filename){
  try{
    ...
    // Desktop & Fallback: open PDF in a new window and trigger the native print dialog
    const url = URL.createObjectURL(blob);
    const w = window.open("", "_blank");
    if (w) {
      w.document.write(`<!doctype html><html><head><title>${filename}</title></head>
        <body style="margin:0">
          <iframe id="printFrame" src="${url}" style="width:100%;height:100vh;border:0"></iframe>
          <script>
      ...
      // If popup blocked, download as fallback
    ...
    console.error("shareOrDownloadPDF failed", e);
    alert("PDF konnte nicht geöffnet werden. Bitte Pop-ups erlauben oder erneut versuchen.");
  }
}
