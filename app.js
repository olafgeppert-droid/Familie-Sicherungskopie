...
async function doLogin(){
  ...
  try{
    ...
    // Check if no vault exists yet
    if(!localStorage.getItem(STORAGE_KEY)){
      if(inputVal === "demo"){
        // Create new empty vault with demo password
        await saveVault({ persons: [] }, currentPassword);
        loginMsg.textContent = "Login erfolgreich (neuer Tresor erstellt).";
        document.getElementById("loginOverlay").style.display = "none";
        return;
      } else {
        throw new Error("Kein Vault vorhanden. Bitte mit Standardpasswort 'demo' starten.");
      }
    }
    await loadVault(currentPassword);
