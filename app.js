...
async function doLogin(){
  ...
  if(!pw){ loginMsg.textContent="Bitte Passwort eingeben."; return; }
  try{
    people = await loadVault(pw);
    currentPassword = pw;
    loginOverlay.style.display="none";
    renderAll();
  }catch(e){
    pwdInput.value = ""; loginMsg.textContent = "Falsches Passwort."; pwdInput.focus();
  }
}
...
document.getElementById('savePasswordBtn').addEventListener('click', () => {
  ...
  // hier AES Verschlüsselung / Speicherung
...
// Zurücksetzen-Button Logik
document.getElementById('resetAppBtn').addEventListener('click', () => {
  if(confirm('Möchten Sie wirklich alle gespeicherten Daten und das Passwort löschen?')){
    localStorage.clear();
    alert('App wurde zurückgesetzt. Bitte neu laden.');
    location.reload();
  }
});
